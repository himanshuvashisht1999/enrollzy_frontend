                    ...
                    <div class="swiper-wrapper">
                        @if(isset($testimonials) && count($testimonials) > 0)
                            @foreach($testimonials as $t)
                                @php
                                    // URLs are fully resolved in PageController using BACKEND_URL
            ...
            <div class="container">
                ...
                <div class="text-center mb-5">
                    <div class="heading-with-lines d-flex align-items-center justify-content-center gap-3 mb-3">
                        <span class="heading-line-orange d-none d-md-block"></span>
                        <h2>{{ !empty($secMentors->title) ? $secMentors->title : "Experts" }}</h2>
                        <span class="heading-line-orange d-none d-md-block"></span>
                    </div>
                    <p class="section-subtitle-custom">
                        {!! !empty($secMentors->subtitle) ? $secMentors->subtitle : "Learn from experienced professionals, industry leaders, and academic experts dedicated to student success." !!}
                    </p>
                </div>
                    ...
                    @forelse($mentors as $index => $mentor)
                        @php
                            ...
                            if (!empty($imgPath)) {
                                ...
                                if (str_starts_with($imgPath, 'http')) {
                                    ...
                                    $mPhoto = asset($imgPath);
                                } elseif (file_exists(public_path('storage/' . $imgPath))) {
                                    $mPhoto = asset('storage/' . $imgPath);
                                } else {
                                    $mPhoto = $backendUrl . '/' . ltrim($imgPath, '/');
                                }
                            }
                        ...
                        <div class="col-xl-3 col-lg-4 col-md-6 col-12 mentor-card-col" data-categories="{{ $catAttr }}">
                            <div class="mentor-grid-card">
                                <div class="mentor-card-img-wrapper" style="height: 260px; overflow: hidden; background-color: #f8f9fa;">
                                    <img src="{{ $mPhoto }}" alt="{{ $mName }}" style="width: 100%; height: 260px; object-fit: cover; object-position: top center;" onError="this.src='{{ asset('assets/images/mentor_1.png') }}'">
                                    <a href="{{ route('expert.detail', $mentor->id) }}" class="mentor-view-profile-btn" title="View Profile">
                                        <i class="fa-solid fa-eye"></i>
                                    </a>
                                </div>
                                <div class="mentor-card-content">
                                    <h3 class="mentor-card-name">{{ $mName }}</h3>
                                    <p class="mentor-card-role">{{ $mentor->designation ?? $mentor->role ?? $mentor->professional_headline ?? 'Senior Counselor & Guide' }}</p>
                                    ...
                                    <div class="mentor-booking-footer">
                                        ...
                                        <a href="{{ route('expert.detail', $mentor->id) }}" class="btn-mentor-book text-decoration-none">
